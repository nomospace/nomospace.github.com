/**
 * 日志工具（window.console.log的替代工具）模块
 * @version 0.2
...
define(function(require, exports, module) {
    ...
    //	日志工具样式
        __style = '\
	        #<uispace>{visibility:hidden;position:fixed;top:10px;right:10px;overflow:auto;z-index:100;width:400px;border:1px solid #97abc9;background-color:#fff;font-family:"courier new";}\
	        #<uispace> .ttl{line-height:25px;height:25px;font-size:14px;padding:0 5px;border-bottom:1px solid #97abc9;}\
	        #<uispace> .cnt{height:400px;line-height:22px;overflow-y:auto;font-size:12px;}\
	        #<uispace> .cnt .line{margin:0;}\
	        #<uispace> .cnt .line:nth-child(even){background-color:#eee;}\
	        #<uispace> .btm{line-height:28px;height:28px;border-top:1px solid #97abc9;text-align:center}',
    //	日志工具结构
        __xhtml = '\
			<div class="ttl">日志测试工具</div>\
			<div class="cnt"></div>\
			<div class="btm">\
				<input type="button" value="清空"/><input type="button" value="关闭"/>\
			</div>';
    /**
     * 日志工具类
     * @constructor
     ...
     */
    var Logger = function(_parent, _options) {
        this._$initialize(_parent, _options);
    };
    Logger.prototype = {
        ...
        __intXnode: function() {
            ...
            _ntmp[0].onclick = function() {
                _me.__cntCase.innerHTML = '';
            };
            _ntmp[1].onclick = function() {
                _me.__body.style.visibility = 'hidden';
            }
        },
        /**
         * 打印日志
        ...
        _$append: function(_message) {
            var _div = document.createElement('pre');
            _div.className = 'line';
            _div.innerHTML = _message + '';
            this.__cntCase.appendChild(_div);
            this.__cntCase.scrollTop = 99999;
        },
        /**
         * 显示日志
         * @return {Void}
     ...
     * @return {Void}
     */
    Logger.show = function(_message, _options) {
